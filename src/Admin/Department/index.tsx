...
const DepartmentPage: React.FC = () => {
  ...
  const { mutateAsync: updateDepartment } = useMutation({
    mutationFn: (body: Department) =>
      getMutationMethod("PUT", `api/departments/${body.id}`, body, true),
    onSuccess: () => {
      fetchDepartments(departmentFilter);
      showToast("Department successfully updated", "success");
    },
    onError: async (error: any) => {
      console.log(error?.message);
      showToast("Department update unsuccessful", "error");
    },
  });

  const { mutateAsync: deleteDepartment } = useMutation({
    mutationFn: (id: number) =>
      getMutationMethod("DELETE", `api/departments/${id}`, {}, true),
    onSuccess: () => {
      fetchDepartments(departmentFilter);
    ...
    onError: async (error: any) => {
      console.log(error?.message);
      showToast("Department deletion unsuccessful", "error");
    },
  });
              ...
              <tr>
                ...
                >
                  <div className="flex items-center">
                    ...
                    {sortConfig?.key === "name" && (
                      <i
                        className={`fas fa-chevron-${sortConfig.direction === "ascending" ? "up" : "down"
                          } ml-1`}
                      ></i>
                    )}
                  </div>
